...
export default {
    ...
    // modulePaths для работы с абсолютными импортами, где rootDir корень проект и добавляем папку src
    // потому что там находится все наши файлы связанные с тестами
    modulePaths: [
        '<rootDir>src/',
    ],
    rootDir: '../../',
    testMatch: [
        // регулярка, которая ищет тесты из корня rootDir в папке src
    ...
    // для глобальных переменных, чтобы jest узнал о них
    globals: {
        __IS_DEV__: true,
        __API__: '',
        __PROJECT__: 'jest',
    ...
    // cacheDirectory: "/private/var/folders/q1/4djrc5sn2csdbbvr2bsfz1_c0000gn/T/jest_dx",
